...
const areas = [
  { number: "01", eyebrow: "Diagnostiquer", title: "Route To Market", copy: "Lire le marché, clarifier le potentiel et transformer une ambition en plan d’action mesurable, jusqu’au trade marketing.", tags: ["Trade marketing", "Business plan", "Audit de terrain"] },
  { number: "02", eyebrow: "Rassembler", title: "Évènementiel", copy: "Concevoir et animer des expériences qui réunissent les publics autour d’un moment juste et mémorable.", tags: ["Stands d’exposition", "Team building", "Conférences"] },
  { number: "03", eyebrow: "Orienter", title: "Stratégie de communication", copy: "Simplifier les enjeux de communication avec une approche organique, éclectique et pragmatique.", tags: ["Positionnement", "Créativité", "Accompagnement personnalisé"] },
  { number: "04", eyebrow: "Raconter", title: "Audiovisuel", copy: "Faire passer une idée par l’image, de l’écriture à la production, la post-production et la mise en scène.", tags: ["Spots publicitaires", "Émissions", "VFX & casting"] },
  { number: "05", eyebrow: "Donner forme", title: "Design", copy: "Développer une identité et des concepts de mise en marché qui répondent au besoin réel de la marque.", tags: ["Concept", "Direction artistique", "Réalisation"] },
  { number: "06", eyebrow: "Prolonger", title: "Goodies", copy: "Choisir et faire vivre l’objet publicitaire qui prolonge la marque avec précision, cohérence et sens du détail.", tags: ["Objet de marque", "Sélection", "Finition"] },
];
...
export default function Agency() {
  return <div className="page-shell"><SiteHeader /><main>
    ...
    <section id="agency-expertises" aria-labelledby="agency-expertises-title"><div className="section-pad"><p className="eyebrow">03 / Nos expertises</p><h2 id="agency-expertises-title" className="display-title">La stratégie est le départ.<br /><em>L’impact est la mesure.</em></h2><p className="large-copy">Une lecture ambitieuse et concrète, du premier diagnostic jusqu’à la réalisation, au plus près des usages.</p></div><div className="expertise-list">{areas.map((area) => <article className="expertise-row" key={area.number}><div className="expertise-row-index">{area.number}</div><div><p>{area.eyebrow}</p><h2>{area.title}</h2></div><p className="expertise-row-copy">{area.copy}</p><div className="expertise-tags">{area.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></article>)}</div></section>
